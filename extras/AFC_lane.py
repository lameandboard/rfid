            ...
            with self.mutex:
                if self.printer.state_message == 'Printer is ready' and self._afc_prep_done and self.status != AFCLaneState.TOOL_UNLOADING:
                    ...
                    if self.prep_state and not self.raw_load_state:
                        ...
                        # --- RFID event hook: afc:lane_prep_start ---
                        try:
                            self.printer.send_event("afc:lane_prep_start", self)
                        except Exception:
                            self.logger.debug("afc:lane_prep_start send_event failed", exc_info=True)
                        ...
                        if (self.load_state
                            and self.prep_state):
                            self.set_loaded()
                            # --- RFID event hook: afc:lane_loaded ---
                            try:
                                self.printer.send_event("afc:lane_loaded", self)
                            except Exception:
                                self.logger.debug("afc:lane_loaded send_event failed", exc_info=True)
                            self.logger.debug(f"RFID: sent afc:lane_loaded for lane={self.name}")
                            self._post_prep_user_macro()
                            # Check if user wants to get TD-1 data when loading
